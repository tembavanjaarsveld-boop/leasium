...
> **2026-06-12 status-semantics note:** Remba (the early UX-reviewer
> persona) was retired and its queue closed as superseded by the
> Horizon Figma approvals (2026-06-10) and the in-loop UX gate. All
> `[~]` markers were promoted to `[x]`; `[~]` is retired. "Pending
> Remba review" sentences in entries below are historical wording —
> do not re-queue work from them. Open UX work lives in the UX Debt
...
- [x] **2026-06-29 Lease billing schedule UX v1:** Property Billing now frames
      ...
      exposes `Starts` and `Ends` lifecycle dates, includes promotion levy and
      utilities/solar charge presets, sends start/end dates to the existing
      charge-rule API, and warns that duplicate base rent should usually be handled
      as a future replacement line for rent increases. Billing Readiness remains the
      invoice-run checkpoint; no tenant email, Xero, payment, reconciliation, or
...
- [x] **2026-06-26 Properties calendar removal:** Calendar is consolidated under
  the Work hub. The Properties overview now exposes Cards, Table, and Map only;
  legacy `/properties?view=calendar` URLs fall back to Cards, and Work calendar
  lease source links open the property record without the removed Properties
  calendar view. Provider guardrails unchanged: this is navigation cleanup over
  the existing read-only calendar endpoint.
- [x] **2026-06-20 AI Mailbox property/task/owner-admin promotion clarity v1:**
  The reviewed mailbox plan card now uses variant-specific review-first copy
  for `property_update`, `task_or_reminder`, and `owner_or_entity_admin`
  promotions. It tells operators whether the action creates a Smart Intake
  review or local Operations task, and explicitly says no property, owner,
  entity, email/SMS, contractor dispatch, provider, payment, or reconciliation
  action happens from the mailbox promotion. Smoke coverage locks all three
  local target variants plus the full AI Mailbox flow.
...
- [x] **2026-06-18 Activity Audit moved to Settings:** Dashboard no longer
  ...
  the existing append-only audit log with a `since_days=60` window and 60-row
  limit, so Settings keeps a focused 60-day history without turning the
  Dashboard into a long log view. Read-only; provider behavior unchanged.
...
- [x] **2026-06-12 AI Mailbox property/task/owner-admin local target handoff v1:**
  ...
  apply Smart Intake, touch payments, or reconcile anything. The 2026-06-20
  clarity pass closed the historical UX/Remba copy debt for the three local
  target variants with per-kind guardrail copy and 1440/390 screenshot checks.
- [x] **2026-06-12 AI Mailbox compliance/insurance Smart Intake handoff v1:**
  ...
  local uploaded Smart Intake review at
  ...
  touch payments, or reconcile anything. UX debt remains for final review of
  the compliance/insurance promote copy and Smart Intake handoff placement.
- [x] **2026-06-12 AI Mailbox reviewed promote handoff v1:**
  ...
  promotion, or OpenAI re-triage runs from this handoff. UX debt remains for
  the inline action placement/copy and richer promote variants.
- [x] **2026-06-12 AI Mailbox Settings trusted-senders v1:**
  ...
  reconciliation side effects. The panel is tracked in the UX Debt Register for
  the Settings AI mailbox UX pass.
- [x] **2026-06-12 AI Mailbox Intake trust/discard v1:**
  ...
  requires an AI mailbox row quarantined for `sender_not_trusted` with SPF/DKIM
  pass, trusts only the authenticated `from_address`, creates/updates the
  organisation `TrustedSender`, marks the row trusted, and writes audit
  metadata without re-running OpenAI, promoting attachments, or creating a
  generic Comms reply candidate. `POST
  /api/v1/comms/inbound-messages/{id}/discard` marks a mailbox row discarded
  and archived while preserving body/raw-email evidence. `/inbox` shows Trust
  sender only for authenticated untrusted senders and Discard for quarantined
  rows; failed-auth rows cannot be trusted from the quarantine detail. Smoke
  covers queue provenance plus trust/discard, and backend tests cover the
  happy paths and failed-auth rejection. No acknowledgement email, tenant
  email, SendGrid/Twilio provider send, Xero/Basiq, payment, reconciliation,
  Smart Intake apply, OpenAI re-triage, or attachment promotion runs from
  either action. UX debt remains for action placement/copy.
- [x] **2026-06-12 Operating mode moves to platform admin v1:** the account
  operating mode (self-managed owner / managing agent / hybrid) is now set by
  Leasium platform admins, not by clients — clients don't decide what they
  are. New `PATCH /api/v1/platform/organisations/{id}/operating-mode` is
  platform-admin gated, audited, and refuses the reserved platform org; the
  old client-side `PATCH /security/organisation/operating-mode` route and
  schema are removed. `/admin` Clients rows now carry an operating-mode
  selector beside Suspend/Manage operators, and client Settings shows a
  read-only "Account type" line ("Set by Leasium for your account") instead
  of the dropdown. Mode still gates People/Owners surfaces exactly as
  before. Backend coverage: admin happy path + audit, non-admin 403,
  reserved-org 400, unknown-value 422, and removed-route 404. Smoke
  coverage: Settings read-only account type (no dropdown) and /admin
  operating-mode change. In-loop UX pass done at 1440/390 (stacked the
  account-type value/explainer after a 390px wrap finding).
- [x] **2026-06-12 Sidebar user card name-first v1:** the Horizon sidebar
  user card now shows the operator's name first (Clerk `fullName` →
  `display_name` → email) with the account email in small muted type below,
  and drops the static "Owner - operator" role line — matching the updated
  Figma sidebar component (Temba sign-off in-session). Clerk hooks stay
  provider-safe via a split component for dev mode. Smoke asserts
  name + email and the role line's absence. In-loop UX pass done at 1440
  and in the 390 drawer.
- [x] **2026-06-12 AI Mailbox Intake UI foundation v1:**
  `/inbox` now includes a read-only AI Mailbox panel built from the Figma
  `03 Screens / AI Mailbox Intake 82:2` frame: copy-address affordance for
  `ai@leasium.ai`, trusted mailbox queue, quarantine bucket, and selected
  quarantined-email provenance with sender/auth detail, stored body text, and
  raw-email download link. The panel only reads
  `GET /api/v1/comms/inbound-messages`; it does not trust senders, discard
  mail, promote/apply suggestions, send email/SMS, run provider calls, or
  mutate Smart Intake. Smoke coverage locked the initial read-only
  queue/provenance flow. Trust/discard now ships separately above; promote/apply
  remains pending. Pending Remba review.
- [x] **2026-06-12 AI Mailbox Intake foundation v1:**
  ...
  design-approved mailbox product slice.
...
- [x] **2026-06-12 All entities billing drafts fan-out reduction v1:**
  ...
  read scoped to the operator's readable entities, while explicit hidden-entity
  reads still return 403 and omitted-entity `property_id`, `lease_id`, and
  `document_intake_id` filters require the linked record's entity to be
...
- [x] **2026-06-12 All entities directory fan-out reduction v1:**
  ...
  changed.
- [x] **2026-06-12 Vendor portal message notifications v1:** operator
...
- [x] **2026-06-11 Horizon Dashboard mobile first-viewport follow-up:**
...
- [x] **2026-06-11 Horizon Notifications mobile first-viewport follow-up:**
  ...
  workflow mutation path changed. Pending Remba review.
- [x] **2026-06-11 Horizon Settings mobile tab-rail follow-up:** tightened
  ...
  workflow mutation path changed. Pending Remba review.
- [x] **2026-06-11 Horizon Smart Intake mobile landing follow-up:** tightened
  ...
  workflow mutation path changed. Pending Remba review.
- [x] **2026-06-11 Horizon Properties mobile compact board follow-up:**
  ...
  Desktop cards and direct mobile Map URLs keep their existing controls.
  2026-06-26 follow-up: the Properties Calendar entry was removed because the
  proper calendar now lives under Work. Added a Notifications mobile guard
  proving Retry notice, Send SMS, and Send digest actions keep touch-safe
  targets. Frontend-only presentation
  and smoke coverage: no API shape, provider send, email/SMS, Xero/Basiq,
  payment, reconciliation, Smart Intake apply, or workflow mutation path
  changed. Pending Remba review.
- [x] **2026-06-11 Horizon Tenant portal mobile v1:** implemented the approved
  ...
  review.
- [x] **2026-06-11 Horizon Insights v1:** implemented the approved Figma
...
- [x] **2026-06-11 Horizon Money v1:** implemented the approved Figma Money
...
- [x] **2026-06-11 Horizon People v1:** implemented the approved Figma People
  ...
  changed. Pending Remba review.
- [x] **2026-06-11 Horizon Dashboard hero/bento polish v2:** tightened `/`
  ...
  workflow mutation path changed. Pending Remba review.
- [x] **2026-06-11 All entities fresh default v1:** fresh multi-entity
  ...
  payment, reconciliation, Smart Intake apply, or workflow mutation path
  changed. Pending Remba review.
- [x] **2026-06-11 Horizon Dark Mode Token Alignment v1:** aligned the shipped
  ...
  Remba review.
- [x] **2026-06-11 Horizon Mobile Polish v1:** shipped the first mobile polish
...
- [x] **2026-06-11 Horizon Notifications + Settings v1:** implemented the
  ...
  Pending Remba review.
- [x] **2026-06-11 Horizon Document review v1:** implemented the approved
...
- [x] **2026-06-10 Horizon shell polish v1:** tightened the shared operator
  ...
  path changed. Pending Remba review.
- [x] **2026-06-10 Horizon Smart Intake desktop v1:** implemented the approved
...
- [x] **2026-06-10 Horizon Property detail v1:** implemented the approved
  ...
  workflow mutation path changed. Pending Remba review.
- [x] **2026-06-10 Horizon Properties desktop v1:** implemented the approved
  ...
  portfolio summary, Cards/Table/Map segmented switcher, three stat cards,
  property card grid, dashed Add property tile, and review-first trust
  pill. Existing Table, Map, create/edit, image/evidence, document,
  ...
  mutation path changed. Pending Remba review.
- [x] **2026-06-10 Horizon app shell v1:** implemented the approved Horizon
  ...
  no API shape, provider send, Xero/Basiq, payment, reconciliation, or workflow
  mutation path changed. Pending Remba review.
- [x] **2026-06-10 Horizon Work desktop v1:** implemented the approved Figma Work
  ...
  changed. Pending Remba review.
- [x] **2026-06-10 Horizon Dashboard v1:** first implementation pass from the
  ...
  review.
- [x] **2026-06-10 "All entities" rollup pages (Dashboard/Insights/Billing-readiness):**
  ...
  Pending Remba review.
- [x] **2026-06-10 "All entities" composite pages + single-entity guards:** Comms,
  ...
  Pending Remba review.
- [x] **2026-06-10 "All entities" list-page rollout (Operations/Contractors/Inbox/Spreadsheet):**
...
- [x] **2026-06-10 "All entities" shared foundation + People/Tenants:** extracted
  ...
  Pending Remba review.
- [x] **2026-06-10 Properties "All entities" portfolio view:** the entity picker
...
- [x] **2026-06-08 agent push (5 slices):** Portfolio QA reviewed bulk-fix apply
  ...
  managed fallback (migration `20260608_0037`); real Leaflet map on Properties
  (merge-safe manual coordinates; historical month-grid calendar later
  consolidated under Work); dark-mode hardening
...
- [x] Vendor detail direct-read + 404 polish: `GET /api/v1/contractors/{id}` now
  ...
  recovery after load.
- [x] Tenant detail status-aware error polish: `/tenants/[tenantId]` now uses the
...
- [x] Maintenance work-order status-aware error polish:
...
- [x] Property workspace deep-link error polish:
...
- [x] Mobile navigation Escape-close follow-up: the sub-`md` sidebar drawer now closes from the existing global Escape key path, matching command/shortcut overlay behavior while preserving the click/backdrop close paths, left-drawer animation, hub IA, and provider guardrails. Smoke coverage locks the mobile drawer Escape behavior. Pending Remba review with the wider AppHeader/mobile shell pass.
- [x] Command search input touch-target follow-up: the global command search textbox now uses a 44px minimum target, matching the adjacent close button and mobile shell controls while preserving search, command routing, shortcut behavior, palette content, and navigation destinations. Smoke coverage locks the textbox to the 44px target baseline. Pending Remba review with the wider AppHeader/command palette density pass.
- [x] Command search result navigation follow-up: command palette result rows now push the selected route before closing the palette, fixing the Dashboard → Billing command result that could close the palette without leaving the Dashboard. Smoke coverage locks the command result navigation path. Pending Remba review with the wider AppHeader/command palette pass.
- [x] Dashboard onboarding Manage links touch-target follow-up: the Dashboard Onboarding panel "Manage links" action now uses a 44px minimum target while preserving the same Properties handoff and onboarding summary copy. Smoke coverage locks the link to the 44px target baseline. Pending Remba review with the wider command-center density pass.
- [x] Dashboard review-queue action touch-target follow-up: the Dashboard Smart Intake review queue `Review` and `Clear` row actions now use 44px minimum targets while preserving the same Smart Intake handoff, local clear behaviour, and provider/document-apply guardrails. Smoke coverage locks the mocked first review row actions to the target baseline. Pending Remba review with the wider command-center density pass.
...
- [x] Properties inline editor/owner-chip touch-target follow-up: the desktop Properties table inline property-name/street editors, ownership tag filter chips, and filtered-state clear action now use 44px minimum targets while preserving row selection, inline editing, and ownership filtering behavior. Smoke coverage locks the row controls and clear filter to the 44px target baseline. Pending Remba review with the wider Properties workspace density pass.
- [x] Tenants register touch-target follow-up: the desktop Tenant workspace onboarding filters, row drawer-open tenant buttons, and inline contact edit cells now use 44px minimum targets while preserving onboarding filters, drawer behavior, contact editing, and provider guardrails. Smoke coverage locks the register filters and row contact controls to the 44px target baseline. Pending Remba review with the wider Tenant workspace density pass.
...
- [x] Tenant detail document action touch-target follow-up: tenant record document review/download links now use 44px minimum targets across the insurance source summary, document list, lease document row, signed-lease download, and onboarding uploaded-document rows. Smoke coverage locks the review/download links without clicking document downloads, Smart Intake apply, delete, upload, provider send, or tenant mutation paths. Pending Remba review with the wider People record document-action pass.
- [x] Tenant portal preview document-download touch-target follow-up: operator preview document download icon links now use a stable 44px square target while preserving the same document destination, preview CSV behaviour, and no tenant-account/no-send/no-provider/no-document-fetch guardrails. Smoke coverage locks the mocked preview document link to the target baseline without clicking it. Pending Remba review with the wider tenant portal preview density pass.
- [x] Tenant portal invoice PDF touch-target follow-up: signed-in tenant portal invoice PDF controls now use the 44px minimum target baseline while preserving the same account-scoped/token document download paths, invoice visibility, and no payment/provider/document-fetch side effects in smoke. Source smoke coverage locks the gated account-path class because local unauthenticated smoke intentionally stops at the account setup gate. Pending Remba review with the wider tenant portal density pass.
- [x] Tenant portal upload-documents jump touch-target follow-up: the attention header's local `Upload documents` anchor now uses the 44px minimum target baseline while preserving the same in-page `#tenant-documents` handoff and no upload/document/provider side effects. Source smoke coverage locks the gated account-path class because local unauthenticated smoke intentionally stops at the account setup gate. Pending Remba review with the wider tenant portal density pass.
...
- [x] Owner statement PDF export v1: `/api/v1/owners/statements/pdf` now returns a review-only PDF for a selected owner/month, and `/api/v1/owners/statements/pdf-pack` returns a ZIP containing every owner PDF plus a review-only README, accountant manifest CSV, and invoice-evidence CSV for the month. Long property, invoice, Xero, bank reference, and reconciliation evidence rows wrap across lines/pages so accountant review packs stay readable. The Statements page offers authenticated per-owner PDF download, accountant-pack download, finance-checklist CSV download, signoff CSV download, dispatch-review CSV download, dispatch-draft text download, print/save-PDF, and copy-dispatch actions. The generated pack is local to Leasium and does not send owner email, post to Xero, attach files to providers, reconcile payments, or mutate delivery history. Pending Remba/accountant review of statement formatting before owner dispatch is wired.
...
- [x] Properties calendar follow-up tasks v1: `/api/v1/obligations/lease-event-follow-ups` can create missing internal obligation tasks from active/holding-over rent-review and lease-expiry dates in a reviewed forward runway, skipping existing lease/category/date tasks instead of duplicating them. The historical Properties Calendar action created next-90-day tasks from current property filters, with full-portfolio runs left unscoped for large portfolios, refreshing obligations, insights, and rent-roll data. 2026-06-26 follow-up: the visible Properties Calendar entry was removed now that Calendar lives under Work; the backend follow-up endpoint remains provider-inert plumbing until a future operator-approved Work action needs it. Generated lease-calendar tasks are protected by a source-scoped partial unique index plus insert-race skip handling, while manual obligations can still share the same lease/category/date when needed. Guardrails: no email/SMS, provider dispatch, invoice posting, Xero/Basiq sync, payment reconciliation, or lease mutation. Pending Remba review.
...
- [x] **People hub IA.** SHIPPED pending Remba review: `/people` with a live Owners
...
- [x] **Navigation consolidation to 7 hubs.** SHIPPED pending Remba review: sidebar is now
...
- [x] **Owner portal account foundation.** Operator-preview foundation remains in
...
- [x] **Tenant self-serve payments on AU rails.** PayTo / PayID / BPAY / direct debit
...
- [x] Vendor portal (contractors log in, accept jobs, post updates/photos).
...
- [x] Communications hub (templates + logs). The queue, dispatch, outbound log,
...
payment settlement. These are places to _beat_ the standard.
...
- [x] Tier 2 (f) v1: Cross-property activity feed, now Activity Audit in Settings. New `GET /api/v1/activity-feed?entity_id=...&limit=N` endpoint projects the append-only `audit_action` rows for the entity into a presentation-friendly shape — actor name (joined from app_user), action_kind chip, target label (batched lookup for property/tenant/lease/invoice/maintenance/arrears/document_intake/tenant_onboarding/obligation), target deep-link href, summary (uses `tool_output_summary` when written, else synthesised), and outcome. Originally rendered on Dashboard; relocated 2026-06-18 to `/settings?tab=activity` with a 60-day audit window so Dashboard stays focused on current decisions. Read-only — no double-write, no separate feed table; the audit log everything else already writes is the source of truth. Backend integration tests assert audit projection, has_more pagination, access control, and the 60-day filter; smoke fixture mocks `/activity-feed`.
...
- [x] Tier 3 v1 multi-view for Properties: Table (existing) and Board (new) views toggleable from a tab strip above the property list, with `?view=board` URL persistence alongside occupancy + owner_tag filters. Board view groups the filtered property set into occupancy buckets (Leased / Leased internal / Partial / Vacant / No units) — each card shows the property thumbnail, name, address, and next-expiry chip; clicking a card selects the property like the table row does. Map remains the Properties planning follow-up surface; the historical Properties Calendar variant was removed on 2026-06-26 because Calendar now lives under Work, and stale `?view=calendar` URLs fall back to Cards. 2026-06-02 map mobile hardening added matching 390px smoke coverage for `?view=map`, lifted map focus chips to the 44px touch baseline, and clarified the read-only local-planning behavior as a Portfolio location plan with `No units` labels instead of raw `0/0` marker ratios. 2026-06-26 follow-up: address/postcode-only AU rows now render approximate local pins until exact `metadata.map_location` coordinates are saved, with shown/pinned/needs-pin counts and mobile fit padding for the fixed bottom nav. Smoke coverage verifies table/board/map toggles and URL persistence. Pending Remba review.
...
- [ ] Maintenance and arrears workflow depth: tenant-submitted requests, contractor coordination, approval history, invoice linking, legal/escalation notes, and portal-visible status where appropriate.
  - [x] Maintenance review packet v1: work-order detail now has a read-only
  ...
  - [x] Arrears review packet v1: Operations Arrears rows now have a read-only
...
- [x] Compliance and inspections: recurring checks, certifications, document expiry, fire/safety obligations, delegated owners, and audit trail.
...
- [x] Communications hub: email templates, outbound logs, tenant/contractor threads, and record-linked correspondence.
...
Shipped 2026-06-24 — Xero setup stepper: the Settings → Xero "Connect Xero" panel now leads with a guided 3-step stepper (Connect → Auto-map accounts & tax → Match contacts) with active-step highlight and Done/Now/Next status, replacing the flat 1-2-3 cards. Status-only — the existing action buttons are unchanged, so no duplicate CTAs. Follow-ups (operator review): collapse the freshness + exception-queue ops panels behind Advanced; de-dupe the secondary "Xero connection" / "Chart and tax mapping" panels.
...
Merged 2026-07-02 - RELBY-MODEL-001 Phase 3 Smart Intake multi-unit lease proposal: lease review matching now returns existing unit candidates alongside property and tenant candidates. The Relby AI matcher review card proposes one lease linked to multiple existing units when the document references labels such as `T101 T103`, and Apply sends reviewed `tenancy_unit_ids` so one lease gets multiple `lease_unit` links with equal percentage apportionment. Extraction prompts/schema/model choices were not changed. Apply remains the only mutation gate and creates internal records only; no Xero, SendGrid, Twilio, tenant email, payment, or reconciliation path was added.

Implemented 2026-07-02, pending CI/prod proof - RELBY-MODEL-001 Phase 4 itemised unit billing drafts: charge rules now support `split_by_unit` and per-unit amount overrides through metadata-backed typed API fields. Billing Readiness local drafts split one lease invoice into per-unit lines using lease apportionment with exact-cent allocation, and invoice draft lines preserve unit metadata for review. Property Billing and tenant Lease & Billing schedule forms expose Split by unit; Billing Readiness previews the itemised unit lines before invoice approval. Xero itemised unit draft creation is blocked behind `xero_itemised_unit_lines_enabled`, default OFF, and still requires explicit operator approval before any provider write. No Xero write, tenant email, SendGrid, Twilio, payment, or reconciliation action was added or run.

Implemented 2026-07-02, pending CI/prod proof - recurring rental incentive discounts: charge schedules now include a `Rental incentive` type that accepts negative amounts for monthly lease discounts, while ordinary charge types still reject negative amounts. Billing drafts and invoice drafts include the negative line in invoice order after base rent and before outgoings, netting the total instead of blocking readiness. Property Billing and tenant Lease & Billing forms accept signed amounts for schedule lines. Adds a Postgres enum migration for `rental_incentive`. No Xero write, tenant email, SendGrid, Twilio, payment, or reconciliation action was added or run.
