...
- [x] Smart Intake acquisition tenancy schedule v1: purchase contracts can extract reviewed schedule rows, create/link multiple units, preserve tenant/rent/lease facts on unit provenance metadata, and create pending tenant/lease records for complete reviewed rows after Apply.
...
- [ ] Continue AI apply beyond first-pass register workflows: add richer acquisition schedule blockers, lease-task generation, and expand billing drafts toward approval-ready invoice generation.
