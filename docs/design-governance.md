...
- 2026-06-18 · Settings two-pane workspace · reused the approved Horizon
  ...
  `output/playwright/settings-two-pane-mobile.png`) plus mocked smoke coverage
  · fixes: corrected the grouped-picker-only pass into a real Settings module
  where the primary app sidebar stays left, a dedicated Settings category pane
  sits in the middle, and the selected category opens as the right-hand page;
  retained Workspace/Account/Apps grouping, People & access/Integrations
  labels, Notifications ownership of Work notification controls, and
  review-first provider copy · provider guardrails unchanged; no provider,
...
- **Settings Figma frame refresh (2026-06-18):** `/settings` shipped the
  ...
  Settings category pane, right-hand selected category page) against the
...
- **Horizon Properties desktop v1 (2026-06-10, Remba pending):** production
  ...
  review card density, the live summary/stat calculations with demo data,
  unknown rent/occupancy treatment in all-entities mode, and the remaining top
  utility bar difference against the exact Figma crop before this is marked `[x]`.
- **Horizon Property detail v1 (2026-06-10, Remba pending):** production
  implementation from the approved Figma Property detail target (`58:627`) as the
  selected-property state inside `/properties`. The frame adds the property
  header, owner/entity and occupancy chips, Overview/Lease/Billing/Documents/
  Activity tabs, bento metrics, current lease panel, and activity summary. Tabs
  reuse the existing lease/unit, billing-readiness, document/image, evidence, and
  source-history workflows rather than creating new write paths; `Work order`
  is a navigation handoff into Work, not a prefilled creation mutation. No API,
  provider send, payment, reconciliation, Xero/Basiq, Smart Intake apply, or
  workflow mutation path changed. Remba should review record-page density, tab
...
- **Property quick-add drawer route (2026-06-08, prototype mode, no Remba gate):**
  the Cmd-K "Add property" frequent action and Smart Intake setup recovery link now
  open `/properties?action=new`, which launches the existing New property drawer instead
  of landing on the Properties table root. The route removes the transient `action`
...
- **Dashboard review-queue action touch targets (2026-06-08, prototype mode, no Remba gate):**
  the Dashboard Smart Intake review queue `Review` and `Clear` row actions now use the 44px
  minimum target baseline while preserving the same Smart Intake handoff and local clear
  behaviour. No document apply, provider, email, Xero, payment, or intake mutation path
  changed. Remba should review the slightly taller review-row rhythm with the wider Dashboard
  command-center density pass.
...
- **Properties table row edit touch targets (2026-06-07, prototype mode, no Remba gate):**
...
- **Notifications disclosure touch targets (2026-06-07, prototype mode, no Remba gate):**
  the notification-center provider setup and receipt evidence disclosure summaries now use
...
- **Billing Readiness invoice-preview action target (2026-06-08, prototype mode, no Remba gate):**
  the invoice preparation email preview `View invoice preview` action now uses the 44px
  minimum target baseline while staying inside the existing read-only message preview
  panel. The link destination is unchanged and no invoice approval, PDF storage, tenant
...
- **Operator shell contrast follow-up (2026-06-08, prototype mode, no Remba gate):**
  ...
  canvas. The sidebar keyboard-shortcut footer uses slate-300 on navy, the
...
- **Comms compliance reminder consolidation (2026-06-07, prototype mode, no Remba gate):**
  ...
  obligation by mistake. 2026-06-08 follow-up: grouped cards now expose one local source-item
  handoff per included obligation, landing on stable anchors in Operations Compliance. No
  automatic SendGrid/Twilio/provider action runs, and no obligation, evidence, dispatch, or
  provider-history mutation behaviour changed. Remba should review whether the grouped copy is
  clear enough for AP inbox recipients and whether the per-source link rhythm is the right
  drilldown.
...
UX pass — 2026-06-24 · Xero chart/tax auto-map apply. Surface: Settings → Xero chart/tax preview (added "Apply suggested mappings" button + applied/skipped feedback bar). No new design — reuses the approved Button/StatusBadge and muted feedback-bar pattern mirroring the adjacent contact-apply block (already UX-passed). Screens: rendered at desktop width in smoke (1432px); mobile parity inherited from the mirrored contact-apply block. Slop test: n/a (reused components, no new surface). Verified: eslint, tsc, ruff, pytest (33), targeted smoke green. Deferred to UX debt: explicit authed 1440/390 screenshot pass (Clerk-gated). Shipped: 03bec1e.

UX pass — 2026-06-24 · Xero setup stepper. Surface: Settings → Xero "Connect Xero" panel. Replaced the flat 1-2-3 status cards with a guided 3-step stepper (Connect → Auto-map accounts & tax → Match contacts): active-step highlight + Done/Now/Next status. Status-only (existing Connect/Review buttons unchanged → no duplicate CTAs, no smoke collision). Screens reviewed: desktop 1440 + mobile 390 via mocked Playwright screenshots — stepper stacks cleanly to full-width cards, active step highlighted, no overflow. Slop test: pass (design-system cards/badges, no decoration). Verified: eslint, tsc, settings-xero-ux smoke (6), Xero readiness flow (1). Deferred to operator review: collapsing freshness + exception-queue ops panels behind Advanced, and de-duping the lower "Xero connection" / "Chart and tax mapping" panels (risk burying operational info; needs live visual iteration).
...
UX pass — 2026-06-24 · Migrate-tenant v1 thin button. Surface: tenant record → Linked leases (per-lease actions). Added "Set up portal login" when a lease has no onboarding (creates the migrated applied row, skipping the wizard), and "Send login link" + "Copy login link" + a "Login link sent" badge on a migrated applied row. Reuses Button / SecondaryButton / StatusBadge (44px touch targets by construction); no new tokens or components. Screens reviewed: desktop 1440 + mobile 390 via Playwright screenshots (apps/web/test-results/migrate-tenant-desktop-1440.png, migrate-tenant-mobile-390.png) — buttons render in the lease row and stack cleanly full-width on mobile. Slop test: pass. Verified: eslint + tsc clean; migrate-tenant-ux smoke 2 passed. 2026-06-27 follow-up resolved the deferred workflow noise: migrated rows now show imported → portal invite → portal login only, and lease-pack controls are hidden for the existing-tenant path.

UX pass — 2026-06-27 · Smart Intake tenant setup path. Surfaces: Relby AI lease review card + tenant record migrated onboarding workflow. Added an operator choice between Existing tenant, New tenant, and Needs review before Smart Intake apply; existing tenant creates only an internal migrated onboarding row and points next action to portal invite. Reuses existing Button / SecondaryButton / StatusBadge / Select styles; no new tokens or provider actions. Screens reviewed: Relby AI lease review desktop 1440 + mobile 390 (`intake-setup-path-*` captures) and migrated tenant desktop 1440 + mobile 390 (`migrate-tenant-desktop-1440.png`, `migrated-tenant-mobile-390.png`) — setup buttons stack cleanly on mobile, no text overlap, migrated workflow no longer shows lease-pack detours. Slop test: pass. Verified: backend pytest for existing/review setup paths, ruff, frontend eslint, tsc, intake-conversation smoke 10 passed, migrate-tenant-ux smoke 2 passed.
...
UX implementation note — 2026-06-29 · Record tab standardisation. Surface: property detail + People record headers. Standardised record-level tabs on a shared `RecordTabs` primitive: compact rounded rail, active filled pill, inactive pill-buttons, consistent 44px touch target, and `data-ui="record-tabs"` marker for regression coverage. Property detail and Tenant detail now use the same visual language; Owner and Vendor section links keep anchor semantics but render through the same pill rail. Design-source rule added: record-level sections use compact pill tabs, large segmented cards are for workspace modes with descriptions/counts, and plain/underline tabs stay for lower-level filters. Verified: focused property/tenant tab smoke and People record layout smoke.

UX implementation note — 2026-06-29 · Record tab brand colour. Surface: shared `RecordTabs` across property detail and People record headers. Replaced the heavy black/navy active pill with the Relby primary blue and a small teal logo-dot accent inside the active tab. Inactive tabs stay neutral to avoid confusing navigation with status. Updated the record-tab smoke to assert both Property and Tenant active tabs use the brand treatment. Verified: focused property/tenant tab smoke.

UX implementation note — 2026-06-29 · People hub type tab brand colour. Surface: `/people` People type selector. Matched the active Tenants/Owners/Vendors/Prospects pill to the Relby primary blue + teal logo-dot treatment used by record tabs, while keeping inactive tabs neutral. Prospects now shows an explicit `0` count instead of a dash because the empty state is known, not loading. Updated People hub smoke coverage for the active brand state and zero count. Verified: focused People hub smoke.

UX implementation note — 2026-06-29 · Tenant billing progressive disclosure. Surface: tenant record → Lease & Billing. Billing schedule now keeps saved charge lines visible and collapses the schedule-line editor behind a compact Add line pill when lines already exist; empty schedules still expose the editor. The accordion uses the existing ease-leasium timing, `aria-expanded`, hidden-field disabling while collapsed, and keeps success/errors outside the collapsed region. Moved Delete tenant out of the summary area into a bottom danger-zone row as a red rounded pill, retaining the existing confirmation and provider-safe soft-delete behavior. Design-source rule added for saved setup panels: visible saved rows, collapsed editor, explicit Add control. Verified: focused tenant billing smoke.

UX implementation note - 2026-07-02 · Billing schedule date labels + tax type dropdown (RELBY-UX-007). Surface: property Billing charge schedule form and tenant Lease & Billing schedule editor. Relabelled Starts/Ends/Invoice sent/Next due to Charge starts/Charge ends/Next invoice date/Next payment due with approved helper text, grouped the date fields into Schedule and Next cycle fieldsets, and converted Tax type from free text to a Select that stores the existing Xero tax-type code. No API, DB, or payload field names changed; no Xero write/mutation path was added. Static AU Xero tax types are used because the existing live tax-rate path is the chart/tax POST preview, not a simple read-only dropdown source. Legacy tax values render as selected options. Screens reviewed: mocked Playwright desktop 1440 and mobile 390 captures at `output/playwright/billing-schedule-labels-1440.png` and `output/playwright/billing-schedule-labels-390.png`; form controls fit without overlap. Verified locally: frontend tsc, focused property/tenant billing smoke. Broader smoke currently fails outside this slice, so production verification and Shipped status remain pending.

UX implementation note - 2026-07-02 · Smart Intake multi-unit lease proposal (RELBY-MODEL-001 Phase 3). Surface: Relby AI matcher review card. Added a matched-units panel when a lease extraction references multiple unit labels and the existing matcher finds those units under the matched property. Apply now sends reviewed `tenancy_unit_ids`, creating one lease with multiple `lease_unit` links instead of a merged unit label. Reuses existing matcher card, StatusBadge, Button, and warning/accent styles; no new tokens, no extraction changes, and no provider actions. Verified locally: backend document-intake suite, lease/register focused tests, frontend intake-conversation smoke at desktop 1280 and mobile 390, frontend eslint, frontend tsc, backend ruff.

UX implementation note - 2026-07-02 · Itemised unit billing drafts (RELBY-MODEL-001 Phase 4). Surface: Property Billing schedule form, tenant Lease & Billing schedule editor, and Billing Readiness draft review. Added a Split by unit checkbox using the existing checkbox/form styling; Billing Readiness shows a compact Split by unit badge and unit-line labels before invoice approval. Reuses existing Field, Input, StatusBadge, table, and card patterns; no new tokens. Xero itemised unit draft creation stays feature-flagged OFF and blocked for actual provider draft creation. Verified locally: register/xero backend integration files, focused and full touched billing/properties smoke, tenant billing smoke, frontend eslint, frontend tsc, backend ruff.

UX implementation note - 2026-07-02 · Recurring rental incentive discounts. Surface: Property Billing schedule form and tenant Lease & Billing schedule editor. Added Rental incentive to the existing charge-type Select and allowed signed amounts in the existing amount input, so a monthly tenant discount can be entered as a negative schedule line. Billing/invoice draft ordering keeps the incentive directly after base rent and before outgoings. No new visual pattern or token; no provider action is triggered from setup.
