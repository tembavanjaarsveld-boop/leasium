...
Recent (2026-06-24): Xero chart/tax auto-map apply shipped to prod (commit 03bec1e, Vercel READY on leasium.ai). New POST /xero/chart-tax/apply-preview persists reviewed account-code + tax-type suggestions onto charge rules locally (review-first, no Xero call); Settings → Xero chart/tax preview gained an "Apply suggested mappings" action with applied/skipped feedback. Verified: ruff, 33 pytest, eslint, tsc, targeted smoke. Committed in isolation (8 files) via hunk-staging; the parked Workflows slice, operations/models WIP, branding WIP, and mid-edited docs were left untouched. Design brief: docs/xero-onboarding-simplification-brief-2026-06-24.md. Open follow-ups: the fuller connect → auto-map → confirm stepper IA restructure; folding apply into a discrete confirm step if desired.

Recent (2026-06-24): Xero setup stepper shipped to prod — Settings → Xero "Connect Xero" panel now leads with a guided 3-step stepper (Connect → Auto-map accounts & tax → Match contacts), active-step highlight, status-only (existing buttons unchanged). Verified eslint/tsc + settings-xero-ux smoke (6) + Xero readiness flow (1) + desktop 1440/mobile 390 mocked screenshots. Isolated to apps/web/src/app/settings/page.tsx. Deferred for operator review: collapse freshness + exception-queue ops panels behind Advanced, and de-dupe the lower "Xero connection" / "Chart and tax mapping" panels. The connect tab is still sprawling (Connect panel, freshness, exception queue, a legacy Xero connection panel, 4 conditional previews, Basiq, Chart-and-tax panel).
